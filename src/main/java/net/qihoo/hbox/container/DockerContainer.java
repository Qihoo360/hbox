...
public class DockerContainer implements IContainerLaunch {
    ...
    public DockerContainer(HboxContainerId containerId, HboxConfiguration conf) {
    ...
    public boolean isAlive() {
        if (hboxProcess != null && Utilities.isProcessAlive(hboxProcess)) {
            return true;
    ...
    public Process exec(String command, String[] envp, Map<String, String> envs, File dir) throws IOException {
        ...
        String dockerHost = conf.get(HboxConfiguration.HBOX_DOCKER_REGISTRY_HOST);
        String dockerPort = conf.get(HboxConfiguration.HBOX_DOCKER_REGISTRY_PORT);
        String dockerImageName = conf.get(HboxConfiguration.HBOX_DOCKER_IMAGE);
...
//    Utilities.printProcessOutput(hboxProcess);
