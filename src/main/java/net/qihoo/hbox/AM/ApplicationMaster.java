...
public class ApplicationMaster extends CompositeService {
    ...
    private String hboxContainerType;
    ...
    private ApplicationMaster() {
        ...
        System.setProperty(HboxConstants.Environment.HADOOP_USER_NAME.toString(), conf.get("hadoop.job.ugi").split(",")[0]);
        ...
        single = conf.getBoolean(HboxConfiguration.HBOX_MODE_SINGLE, HboxConfiguration.DEFAULT_HBOX_MODE_SINGLE);
        appPriority = conf.getInt(HboxConfiguration.HBOX_APP_PRIORITY, HboxConfiguration.DEFAULT_HBOX_APP_PRIORITY);
        hboxContainerType = conf.get(HboxConfiguration.HBOX_CONTAINER_TYPE, HboxConfiguration.DEFAULT_HBOX_CONTAINER_TYPE);
        ...
        if (applicationAttemptID.getAttemptId() > 1 && (conf.getInt(HboxConfiguration.HBOX_APP_MAX_ATTEMPTS, HboxConfiguration.DEFAULT_HBOX_APP_MAX_ATTEMPTS) > 1)) {
            int maxMem = Integer.valueOf(envs.get(HboxConstants.Environment.HBOX_CONTAINER_MAX_MEMORY.toString()));
    ...
    private void init() {
        ...
        applicationHistoryUrl = conf.get(HboxConfiguration.HBOX_HISTORY_WEBAPP_ADDRESS,
                HboxConfiguration.DEFAULT_HBOX_HISTORY_WEBAPP_ADDRESS) + "/jobhistory/job/"
        ...
        cleanApplication = new Thread(new Runnable() {
            @Override
            public void run() {
                System.clearProperty(HboxConstants.Environment.HADOOP_USER_NAME.toString());
                YarnConfiguration hboxConf = new YarnConfiguration();
                if (hboxConf.getBoolean(HboxConfiguration.HBOX_CLEANUP_ENABLE, HboxConfiguration.DEFAULT_HBOX_CLEANUP_ENABLE)) {
                    ...
                    try {
                        stagingDir.getFileSystem(hboxConf).delete(stagingDir);
                ...
                try {
                    ...
                    logMessage.put(AMParams.APP_TYPE, hboxAppType);
                    ...
                    if (conf.getBoolean(HboxConfiguration.HBOX_TF_BOARD_ENABLE, HboxConfiguration.DEFAULT_HBOX_TF_BOARD_ENABLE)) {
                        ...
                        if (conf.get(HboxConfiguration.HBOX_TF_BOARD_LOG_DIR, HboxConfiguration.DEFAULT_HBOX_TF_BOARD_LOG_DIR).indexOf("hdfs://") == -1) {
                            if (conf.get(HboxConfiguration.HBOX_TF_BOARD_HISTORY_DIR, HboxConfiguration.DEFAULT_HBOX_TF_BOARD_HISTORY_DIR).equals(hboxConf.get(HboxConfiguration.HBOX_TF_BOARD_HISTORY_DIR, HboxConfiguration.DEFAULT_HBOX_TF_BOARD_HISTORY_DIR))) {
                    ...
                    for (Container container : workerContainers) {
                        ...
                        if (tfEvaluator && container.getId().toString().equals(tfEvaluatorContainerId)) {
                            containerMessage.put(AMParams.CONTAINER_ROLE, HboxConstants.EVALUATOR);
                        } else if (chiefWorker && container.getId().toString().equals(chiefWorkerContainerId)) {
                            containerMessage.put(AMParams.CONTAINER_ROLE, HboxConstants.CHIEF);
                        } else {
                            containerMessage.put(AMParams.CONTAINER_ROLE, HboxConstants.WORKER);
                        }
                        HboxContainerStatus status = applicationContext.getContainerStatus(new HboxContainerId(container.getId()));
                        if (status != null) {
                        ...
                        if (applicationContext.getContainersCpuMetrics().get(new HboxContainerId(container.getId())) != null) {
                            ConcurrentHashMap<String, LinkedBlockingDeque<Object>> cpuMetrics = applicationContext.getContainersCpuMetrics().get(new HboxContainerId(container.getId()));
                            containerMessage.put(AMParams.CONTAINER_CPU_METRICS, new Gson().toJson(cpuMetrics));
                        }

                        if (applicationContext.getContainersCpuStatistics().get(new HboxContainerId(container.getId())) != null) {
                            ConcurrentHashMap<String, List<Double>> cpuStatistics = applicationContext.getContainersCpuStatistics().get(new HboxContainerId(container.getId()));
                            ...
                            if (cpuStatistics.size() != 0) {
                                ...
                                if (status != null && status.toString().equalsIgnoreCase("SUCCEEDED") && cpuMemUsagedMax * 1024.0 / currentWorkerMemory < conf.getDouble(HboxConfiguration.HBOX_CONTAINER_MEM_USAGE_WARN_FRACTION, HboxConfiguration.DEFAULT_HBOX_CONTAINER_MEM_USAGE_WARN_FRACTION)) {
                    ...
                    for (Container container : psContainers) {
                        ...
                        if (applicationContext.getContainersCpuStatistics().get(new HboxContainerId(container.getId())) != null) {
                            ...
                            if (cpuStatistics.size() != 0) {
                                Double cpuMemUsagedMax = cpuStatistics.get("CPUMEM").get(1);
                                if (status != null && status.toString().equalsIgnoreCase("SUCCEEDED") && cpuMemUsagedMax * 1024.0 / workerMemory < conf.getDouble(HboxConfiguration.HBOX_CONTAINER_MEM_USAGE_WARN_FRACTION, HboxConfiguration.DEFAULT_HBOX_CONTAINER_MEM_USAGE_WARN_FRACTION)) {
                                    containerMessage.put(AMParams.CONTAINER_CPU_USAGE_WARN_MEM, "true");
                                } else {
                                    containerMessage.put(AMParams.CONTAINER_CPU_USAGE_WARN_MEM, "false");
                                }
    ...
    private void buildOutputLocations() {
        ...
        if (outputs != null && outputs.length > 0) {
            ...
            throw new RuntimeException("Error output path format " + hboxOutputs);
        }
    }
    ...
    private void buildContainerRequest(String[] hostLocals) {
        if (conf.getBoolean(HboxConfiguration.HBOX_HOST_LOCAL_ENABLE, HboxConfiguration.DEFAULT_HBOX_HOST_LOCAL_ENABLE)) {
            ...
            String hostLocaldir = xlConf.get("fs.defaultFS") + conf.get(HboxConfiguration.HBOX_HISTORY_LOG_DIR,
                    HboxConfiguration.DEFAULT_HBOX_HISTORY_LOG_DIR) + "/" + conf.get("hadoop.job.ugi").split(",")[0]
                    + "/" + envs.get(HboxConstants.Environment.HBOX_APP_NAME.toString());
        ...
        String workerNodeLabelExpression = conf.get(HboxConfiguration.HBOX_WORKER_NODELABELEXPRESSION);
        ...
        if (psNum > 0) {
            String psNodeLabelExpression = conf.get(HboxConfiguration.HBOX_PS_NODELABELEXPRESSION);
        ...
        try {
            ...
            containerLocalResource.put(HboxConstants.HBOX_JOB_CONFIGURATION,
                    Utilities.createApplicationResource(appConfRemoteLocation.getFileSystem(conf),
                            ...
                            LocalResourceType.FILE));
            if (hboxAppType.equals("MPI")) {
                reLinkFiles.append(HboxConstants.HBOX_JOB_CONFIGURATION).append(",");
            }
    ...
    private Map<String, String> buildContainerEnv(String role) {
        ...
        if (outputIndex >= 0) {
            containerEnv.put(HboxConstants.Environment.HBOX_OUTPUTS_WORKER_INDEX.toString(), String.valueOf(outputIndex));
        }
        ...
        if (hboxAppType.equals("MXNET") && !single) {
        ...
        containerEnv.put("LD_LIBRARY_PATH", System.getenv("LD_LIBRARY_PATH") + ":" + System.getenv(HboxConstants.Environment.USER_LD_LIBRARY_PATH.toString()));
        ...
        if (conf.get(HboxConfiguration.HBOX_CONTAINER_EXTRAENV) != null) {
            String[] containerUserEnv = StringUtils.split(conf.get(HboxConfiguration.HBOX_CONTAINER_EXTRAENV), "|");
        ...
        if (hboxContainerType.equalsIgnoreCase("DOCKER")) {
            String dockeImage = conf.get(HboxConfiguration.HBOX_DOCKER_IMAGE);
            int containerMemory = conf.getInt(HboxConfiguration.HBOX_WORKER_MEMORY,
                    HboxConfiguration.DEFAULT_HBOX_WORKER_MEMORY);
            int containerCpu = conf.getInt(HboxConfiguration.HBOX_WORKER_VCORES,
                    HboxConfiguration.DEFAULT_HBOX_WORKER_VCORES);
    ...
    private boolean run() throws IOException, NoSuchAlgorithmException {
        ...
        if (conf.get(HboxConfiguration.HBOX_INPUT_STRATEGY, HboxConfiguration.DEFAULT_HBOX_INPUT_STRATEGY).equals("STREAM")) {
            buildInputStreamFileStatus();
        } else {
            buildInputFileStatus();
        }

        if ("TENSORFLOW".equals(hboxAppType) || "MXNET".equals(hboxAppType) || "LIGHTLDA".equals(hboxAppType) || "XFLOW".equals(hboxAppType)) {
        ...
        try {
            if (!hboxAppType.equals("MPI")) {
                ...
                while (!containerListener.isTrainCompleted()) {
                    ...
                    if (conf.getBoolean(HboxConfiguration.HBOX_REPORT_CONTAINER_STATUS, HboxConfiguration.DEFAULT_HBOX_REPORT_CONTAINER_STATUS)) {
            ...
            if (finalSuccess) {
                if ((conf.get(HboxConfiguration.HBOX_OUTPUT_STRATEGY, HboxConfiguration.DEFAULT_HBOX_OUTPUT_STRATEGY).equals("STREAM")) && outputInfos.size() > 0) {
                    ...
                    for (OutputInfo outputInfo : outputInfos) {
                        ...
                        if (outputIndex >= 0) {
                            ...
                            if (psNum > 0 && (hboxAppType.equals("TENSORFLOW") || hboxAppType.equals("LIGHTLDA"))) {
