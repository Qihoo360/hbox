...
public class HsController extends Controller implements AMParams {
  ...
  public void job() throws IOException {
    ...
    jobLogPath = new Path(conf.get(HboxConfiguration.HBOX_HISTORY_LOG_DIR,
      ...
      for (String info : readLog.keySet()) {
        if (info.equals(AMParams.APP_TYPE)) {
          if (readLog.get(info) != null) {
            if (readLog.get(info).equals("HBOX")) {
              set(APP_TYPE, "Hbox");
            } else {
          ...
          if (readLog.get(info) instanceof Map<?, ?>) {
            ...
            if (containerMessage.get(AMParams.CONTAINER_ROLE).equals(HboxConstants.EVALUATOR) || containerMessage.get(AMParams.CONTAINER_ROLE).equals(HboxConstants.CHIEF)) {
              set(CONTAINER_ROLE + i, HboxConstants.WORKER + "/" + containerMessage.get(AMParams.CONTAINER_ROLE));
            } else {
            ...
            if (containerMessage.containsKey(AMParams.CONTAINER_CPU_METRICS)) {
              ...
              if (cpuMetrics != null) {
                ...
                if (map.size() > 0) {
                  cpuMetricsFlag = true;
                  if (containerMessage.get(AMParams.CONTAINER_ROLE).equals(HboxConstants.WORKER) || containerMessage.get(AMParams.CONTAINER_ROLE).equals(HboxConstants.EVALUATOR) || containerMessage.get(AMParams.CONTAINER_ROLE).equals(HboxConstants.CHIEF)) {
