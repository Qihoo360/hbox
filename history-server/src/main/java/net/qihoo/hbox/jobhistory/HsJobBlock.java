...
public class HsJobBlock extends HtmlBlock implements AMParams {
    ...
    protected void render(Block html) {
        ...
        if (numAMattempt > 0) {
            Hamlet.TBODY<Hamlet.TABLE<Hamlet>> tbody = html.h2("ApplicationMaster")
                    .table("#AMContainers")
        ...
        if (numContainers > 0) {
            Hamlet.TBODY<Hamlet.TABLE<Hamlet>> tbody = html.h2("All Containers:")
                    .table("#Containers")
                    .thead("ui-widget-header")
                    .tr()
            ...
            for (int i = 0; i < numContainers; i++) {
                Hamlet.TD<Hamlet.TR<Hamlet.TBODY<TABLE<Hamlet>>>> td = tbody.__()
                        .tbody("ui-widget-content")
                        .tr()
                        .$style("text-align:center;")
                        .td();
                td.span()
                        .$title(String.format($(CONTAINER_ID + i)))
                        .__()
                        .a($(CONTAINER_LOG_ADDRESS + i), String.format($(CONTAINER_ID + i)));
                ...
                if ($(CONTAINER_REPORTER_PROGRESS + i).equals("progress log format error")) {
                    td.__()
                            ...
                            .td($(CONTAINER_REPORTER_PROGRESS + i))
                            .__();
                    ...
                    td.__()
                            ...
                            .$title($(CONTAINER_REPORTER_PROGRESS + i))
                            .div()
                            .$class("ui-progressbar-value ui-widget-header ui-corner-left")
                            .$style("width:" + $(CONTAINER_REPORTER_PROGRESS + i))
                            .__()
                            .__()
                            .__()
                            .__();
            ...
            if (Boolean.parseBoolean($("USAGED_INFO"))) {
                ...
                Hamlet.TBODY<TABLE<Hamlet>> resourceAppliedInfo = html.h2("Resource Applied Info:")
                ...
                if (numWorkers > 0) {
                    Hamlet.TBODY<TABLE<Hamlet>> workerCPUUsage = html.h2("Worker Containers CPU Usage Info:")
                    ...
                    for (int i = 0; i < numWorkers; i++) {
                        ...
                        if (memWarn != null && memWarn.length() > 0 && Boolean.valueOf(memWarn)) {
                            ...
                            td.__($("worker" + CPU_USAGE_TYPE + CONTAINER_MEM_USAGE_STATISTICS + USAGE_MAX + i));
                        }
                        td.__()
                                .td($("worker" + CPU_USAGE_TYPE + CONTAINER_UTIL_USAGE_STATISTICS + USAGE_AVG + i))
                    ...
                    if (workerGcores > 0) {
                        html.div().$style("margin:20px 2px;").__(" ").__();
                        Hamlet.TBODY<TABLE<Hamlet>> workerGPUUsage = html.h2("Worker Containers GPU Usage Info:")
                            ...
                            for (int j = 0; j < gpusIndex.length; j++) {
                                workerGPUUsage
                                        ...
                                        .td($("worker" + GPU_USAGE_TYPE + CONTAINER_MEM_USAGE_STATISTICS + USAGE_MAX + i
                                                + gpusIndex[j]))
                                        .td($("worker" + GPU_USAGE_TYPE + CONTAINER_UTIL_USAGE_STATISTICS + USAGE_AVG
                                                + i + gpusIndex[j]))
                                        .td($("worker" + GPU_USAGE_TYPE + CONTAINER_UTIL_USAGE_STATISTICS + USAGE_MAX
                                                + i + gpusIndex[j]))
                                        .__();
                            }
                        }
                ...
                if (numPS > 0) {
                    html.div().$style("margin:20px 2px;").__(" ").__();
                    Hamlet.TBODY<TABLE<Hamlet>> psCPUUsage = html.h2("PS Containers CPU Usage Info:")
                    ...
                    for (int i = 0; i < numPS; i++) {
                        Hamlet.TD<Hamlet.TR<Hamlet.TBODY<TABLE<Hamlet>>>> td = psCPUUsage
                                ...
                                .td($(CONTAINER_ROLE + (numWorkers + i)))
                                .td($(CONTAINER_RANK + (numWorkers + i)))
                                .td($("ps" + CPU_USAGE_TYPE + CONTAINER_MEM_USAGE_STATISTICS + USAGE_AVG + i))
                                .td();
                        ...
                        if (memWarn != null && memWarn.length() > 0 && Boolean.valueOf(memWarn)) {
                            td.$style("color:red")
                                    .b(String.format(
                                            "%s\t( Current cpu memory used is much less than applied. Please adjust !! )",
                                            $("ps" + CPU_USAGE_TYPE + CONTAINER_MEM_USAGE_STATISTICS + USAGE_MAX + i)));
                        } else {
                            td.__($("ps" + CPU_USAGE_TYPE + CONTAINER_MEM_USAGE_STATISTICS + USAGE_MAX + i));
                        }
                        td.__()
                                .td($("ps" + CPU_USAGE_TYPE + CONTAINER_UTIL_USAGE_STATISTICS + USAGE_AVG + i))
                    ...
                    if (PSGcores > 0) {
                        html.div().$style("margin:20px 2px;").__(" ").__();
                        Hamlet.TBODY<TABLE<Hamlet>> psGPUUsage = html.h2("PS Containers GPU Usage Info:")
                        ...
                        for (int i = 0; i < numPS; i++) {
                            ...
                            for (int j = 0; j < gpusIndex.length; j++) {
                                psGPUUsage
                                        ...
                                        .td($(CONTAINER_ROLE + (numWorkers + i)))
                                        .td($(CONTAINER_RANK + (numWorkers + i)))
                                        .td($("ps" + GPU_USAGE_TYPE + CONTAINER_MEM_USAGE_STATISTICS + USAGE_AVG + i
                                                + gpusIndex[j]))
                                        .td($("ps" + GPU_USAGE_TYPE + CONTAINER_MEM_USAGE_STATISTICS + USAGE_MAX + i
                                                + gpusIndex[j]))
                                        .td($("ps" + GPU_USAGE_TYPE + CONTAINER_UTIL_USAGE_STATISTICS + USAGE_AVG + i
                                                + gpusIndex[j]))
                                        .td($("ps" + GPU_USAGE_TYPE + CONTAINER_UTIL_USAGE_STATISTICS + USAGE_MAX + i
                                                + gpusIndex[j]))
                                        .__();
                            }
                        }
            ...
            if (timestampSize > 0) {
                html.div().$style("margin:20px 2px;").__(" ").__();
                Hamlet.TBODY<TABLE<Hamlet>> tbodySave = html.h2("Saved Model")
                        .table("#savedmodel")
                        .thead("ui-widget-header")
                        .tr()
                ...
                for (int i = 0; i < timestampSize; i++) {
                    String timeStamp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                            .format(new Date(Long.parseLong($(TIMESTAMP_LIST + i))));
                    ...
                    String pathStr = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss")
                            .format(new Date(Long.parseLong($(TIMESTAMP_LIST + i))));
                ...
                if (PSGcores > 0) {
                    for (int i = 0; i < numPS; i++) {
                        if (!$("pscpuMemMetrics" + i).equals("") && $("pscpuMemMetrics" + i) != null) {
                            html.div()
                                    .$style("margin:20px 2px;font-weight:bold;font-size:12px")
                                    .__(String.format($("PS_CONTAINER_ID" + i)) + " metrics:")
                                    .__();
                            if (!injectedHighstockJS) {
                                html.script()
                                        .$src("/static/hboxWebApp/jquery-3.1.1.min.js")
                                        .__();
                                html.script()
                                        .$src("/static/hboxWebApp/highstock.js")
                                        .__();
                                html.script()
                                        .$src("/static/hboxWebApp/exporting.js")
                                        .__();
                            ...
                            html.div()
                                    .div()
                                    .$id(containerGpuMemID)
                                    .$class(containerClass)
                                    .$style("height: 400px; min-width: 310px; diplay:inline-block")
                                    .__()
                                    .div()
                                    .$id(containerGpuUtilID)
                                    .$class(containerClass)
                                    .$style("height: 400px; min-width: 310px; diplay:inline-block")
                                    .__()
                                    .div()
                                    .$id(containerCpuMemID)
                                    .$class(containerClass)
                                    .$style("height: 400px; min-width: 310px; diplay:inline-block")
                                    .__()
                                    .div()
                                    .$id(containerCpuUtilID)
                                    .$class(containerClass)
                                    .$style("height: 400px; min-width: 310px; diplay:inline-block")
                                    .__()
                                    .__();
                            ...
                            String striptHead = "Highcharts.setOptions({\n" + "    global: {\n"
                                    ...
                                    + "// Create the chart\n";
                            String striptBody =
                                    "Highcharts.stockChart(" + containerGpuMemID + ", {\n" + "    chart: {\n"
                                            + "        width: 550\n"
                                            + "    },\n"
                                            + "\n"
                                            + "    rangeSelector: {\n"
                                            ...
                                            + "\n"
                                            + "    series: "
                                            + seriesCpuUtilOptions + "\n" + "});\n";

                            html.script()
                                    .$type("text/javascript")
                                    .__(striptHead + striptBody)
                                    .__();
                        }
                    }
                } else {
                ...
                if (workerGcores > 0) {
                    for (int i = 0; i < numWorkers; i++) {
                        if (!$("workercpuMemMetrics" + i).equals("") && $("workercpuMemMetrics" + i) != null) {
                            html.div()
                                    .$style("margin:20px 2px;font-weight:bold;font-size:12px")
                                    .__(String.format($("WORKER_CONTAINER_ID" + i)) + " metrics:")
                                    .__();
                            if (!injectedHighstockJS) {
                                html.script()
                                        .$src("/static/hboxWebApp/jquery-3.1.1.min.js")
                            ...
                            String seriesCpuMemOptions = "[{\n" + "            name: 'cpu mem used',\n"
                                    + "            data: "
                                    + $("workercpuMemMetrics" + i) + "\n" + "        }]";
                            String seriesCpuUtilOptions = "[{\n" + "            name: 'cpu util',\n"
                                    + "            data: "
                                    + $("workercpuUtilMetrics" + i) + "\n" + "        }]";
                            html.div()
                                    .div()
                                    .$id(containerGpuMemID)
                                    .$class(containerClass)
            ...
            html.div()
                    ...
                    .__("Not get the container information, please check all containers started successfully !")
                    .__();
        }
    }
}
