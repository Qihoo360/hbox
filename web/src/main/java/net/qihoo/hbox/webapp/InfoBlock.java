...
public class InfoBlock extends HtmlBlock implements AMParams {
    ...
    protected void render(Block html) {
        ...
        if (numContainers > 0) {
            TBODY<TABLE<Hamlet>> tbody = html.h2("All Containers:")
                    .table("#Containers")
            ...
            for (int i = 0; i < numContainers; i++) {
                TD<TR<TBODY<TABLE<Hamlet>>>> td = tbody.__()
                ...
                td.span()
                        ...
                        .a(
                                String.format(
                                        ...
                                        $(CONTAINER_HTTP_ADDRESS + i), $(CONTAINER_ID + i), $(USER_NAME)),
            ...
            if (!$(BOARD_INFO).equals("no")) {
                if (!$(BOARD_INFO).contains("http")) {
                    ...
                    tbody.__()
                            ...
                            .td()
                            .span()
                            .$title(String.format($(BOARD_INFO)))
                            .__()
                            .a(String.format($(BOARD_INFO)), String.format($(BOARD_INFO)))
                            .__()
                            .__()
                            .__()
                            .__();
            ...
            TBODY<TABLE<Hamlet>> resourceAppliedInfo = html.h2("Resource Applied Info:")
            ...
            if (numWorkers > 0) {
                TBODY<TABLE<Hamlet>> workerCPUUsage = html.h2("Worker Containers CPU Usage Info:")
                ...
                if (workerGcores > 0) {
                    html.div().$style("margin:20px 2px;").__(" ").__();
                    TBODY<TABLE<Hamlet>> workerGPUUsage = html.h2("Worker Containers GPU Usage Info:")
                    ...
                    for (int i = 0; i < numWorkers; i++) {
                        ...
                        for (int j = 0; j < gpusIndex.length; j++) {
                            workerGPUUsage
                                    ...
                                    .td(gpusIndex[j])
                                    .td($(GPU_USAGE_TYPE
                                            + CONTAINER_MEM_USAGE_STATISTICS
                                            + USAGE_AVG
                                            + i
                                            + gpusIndex[j]))
                                    .td($(GPU_USAGE_TYPE
                                            ...
                                            + gpusIndex[j]))
                                    .td($(GPU_USAGE_TYPE
                                            + CONTAINER_UTIL_USAGE_STATISTICS
                                            + USAGE_AVG
                                            + i
                                            + gpusIndex[j]))
                                    .td($(GPU_USAGE_TYPE
            ...
            if (numPS > 0) {
                html.div().$style("margin:20px 2px;").__(" ").__();
                TBODY<TABLE<Hamlet>> psCPUUsage = html.h2("PS Containers CPU Usage Info:")
                ...
                if (psGcores > 0) {
                    html.div().$style("margin:20px 2px;").__(" ").__();
                    TBODY<TABLE<Hamlet>> psGPUUsage = html.h2("PS Containers GPU Usage Info:")
            ...
            if ((saveModelTotal > 0) && (saveModelSize > 0)) {
                if (!Boolean.valueOf($(SAVE_MODEL))) {
                    ...
                    script.append("function savedModel(){")
                            ...
                            .append("document.location.href='/proxy/")
                            .append($(APP_ID))
                    ...
                    if (!$(SAVE_MODEL_STATUS).equals($(SAVE_MODEL_TOTAL))) {
                        html.div()
                                .$style("margin:20px 2px;")
                                .__(String.format(
                                        "saving the model ... %s/%s", $(SAVE_MODEL_STATUS), $(SAVE_MODEL_TOTAL)))
                                .__();
                    } else {
                        StringBuilder script = new StringBuilder();
                        script.append("location.href='/proxy/")
                                .append($(APP_ID))
                                .append("';");
                        html.script()
            ...
            if (modelSaveTotal > 0) {
                TBODY<TABLE<Hamlet>> tbodySave = html.h2("")
                        .table("#savedmodel")
                        .thead("ui-widget-header")
                        .tr()
                ...
                for (i = 0; i < numWorkers; i++) {
                    if (!$("cpuMemMetrics" + i).equals("") && $("cpuMemMetrics" + i) != null) {
                        ...
                        if (!injectedHighstockJS) {
                            html.script()
                                    .$src("/proxy/" + $(APP_ID) + "/static/hboxWebApp/jquery-3.1.1.min.js")
                                    .__();
                            html.script()
                                    .$src("/proxy/" + $(APP_ID) + "/static/hboxWebApp/highstock.js")
                                    .__();
                            html.script()
                                    .$src("/proxy/" + $(APP_ID) + "/static/hboxWebApp/exporting.js")
                                    .__();
            ...
            html.div()
                    ...
                    .__("Waiting for all containers allocated......")
                    .__();
        }
    }
}
